...
from blindtest.youtube_search.typings import TypeBaseSearch


class Query[T = TypeBaseSearch]:
...
class QueryDict(Query):
    ...
    def filter(self, query_path: str):
        """Function used to traverse the keys within a
        dictionnary returning the specific value stored

        >>> value = {'name': {'firstname': {'eu': 'Paul'}}}
        ... instance = QueryDict(value)
        ... instance.get('name__firstname')
        {'eu': 'Paul'}
        """
        ...
        for key in self.keys:
            if queried_data is None:
                ...
                if is_valid:
                    ...
                    # prefer a list iterator: QueryList
                    queried_data = value
                    break
    ...
    def check(self, key: str, data: Any):
        ...
        if dataclasses.is_dataclass(data):
            data: dict[str, Any] = dataclasses.asdict(data)
...
class ResultsIterator[T = TypeBaseSearch, R = list]:
    def __init__(self):
        self.search_instance: Optional[T] = None
